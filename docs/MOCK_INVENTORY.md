...
| 7 | `src/shared/mock/data.ts` | `mockModels` | Hardcoded GGUF model entries | Load from `aisec-models` registry via IPC |
...
| 19 | `src-tauri/Cargo.toml` | dependencies | Only `aisec-core`; no storage/engines | Add `aisec-storage`, `aisec-discovery`, `aisec-attack`, `aisec-judge`, `aisec-report` |
...
| 6 | `src/features/attacks/AttacksPage.tsx` | `attackCategories` constant | Hardcoded 9 categories in TS, not from `aisec-attack` registry | Optional: fetch from backend; MVP uses `scan_run` only |
...
| 12 | `src/features/models/ModelsPage.tsx` | "Download Model" / "Browse HuggingFace" | No handler | `aisec-models` IPC (post-MVP) |
...
|------|-----------------|------------------|---------------------|
| `crates/aisec-attack/src/transport/mock.rs` | `MockTransport`, `MockTransport::ok()`, `send()` | Returns canned HTTP responses; records requests for assertions | **`HttpTransport`** (`transport/http.rs`) — already implemented for production |

### C2. Auth Playwright mock

| File | Type / function | Why it is a mock | Real implementation |
|------|-----------------|------------------|---------------------|
| `crates/aisec-auth/src/mock.rs` | `MockPlaywrightDriver`, `login_success()` | Returns fixed cookies/tokens without Node/Playwright | **`PlaywrightClient`** (`playwright/client.rs`) + `runner.mjs` subprocess |

### C3. Inference runtime mocks

| File | Type / function | Why it is a mock | Real implementation |
|------|-----------------|------------------|---------------------|
| `crates/aisec-models/src/runtime/mock.rs` | `MockInferenceRuntime`, `complete()` | Returns `"[mock: prompt]"` text | **`LlamaCppRuntime`** (`runtime/llama_cpp.rs`) |
| `crates/aisec-judge/src/mock_runtime.rs` | `JsonMockRuntime`, `judge_vulnerable()`, `classifier()` | Returns fixed JSON verdict strings for judge tests | **`LlamaCppRuntime`** via `ModelRolePool` or **`judge_deterministic()`** for MVP |

### C4. Storage test database

| File | Function | Why it is a mock | Real implementation |
|------|----------|------------------|---------------------|
| `crates/aisec-storage/src/pool.rs` | `test_utils::test_database()` | In-memory SQLite (`sqlite::memory:`) | File-backed `Database::connect("sqlite://~/.aisec/aisec.db")` in Tauri |
...
| File | Function | Why it is a mock | Real implementation |
|------|----------|------------------|---------------------|
| `crates/aisec-plugin-host/src/manager.rs` (test) | writes `plugin.py` with `print('stub')` | Minimal fake plugin for discovery test | Real plugins in `plugins/samples/` |
...
| 2 | `crates/aisec-discovery/tests/integration.rs` | `MockServer` (wiremock) | In-process HTTP mock for discovery integration test | Live target or test fixture server |
| 3 | `crates/aisec-models/tests/integration.rs` | `MockServer` | Mocks HuggingFace download URLs | Real HuggingFace CDN |
| 4 | `crates/aisec-models/src/download/manager.rs` (tests) | `MockServer` | Download/resume unit tests | Real HTTP download |
| 5 | `crates/aisec-attack/tests/integration.rs` | Uses `MockTransport` (not HTTP server) | In-memory response fake | `HttpTransport` to real API |
...
| 4 | `plugins/samples/judge-keyword/plugin.py` | `evaluate()` | Keyword stub judge (not `aisec-judge`) | `aisec-judge` engine or plugin hook to judge |
| 5 | `plugins/samples/report-markdown-summary/plugin.js` | Report hook | Sample markdown formatter | `aisec-report` HTML/PDF pipeline |
| 6 | `plugins/samples/*/aisec-plugin.toml` | manifest | Reference manifests (currently **incompatible** with host parser) | Align schema + enable via Plugin Manager |
...
|------|------|--------|-----|---------------------|
| `crates/aisec-discovery/examples/verify_target.rs` | 15 | `TODO: use worker_count 4+ after crawler deadlock fix` | Workaround forces single worker | Fix crawler notify deadlock in `crawler.rs` |
...
|------|------|-------------|
| `{{payload}}` placeholder | `aisec-attack/src/types.rs` | Template variable in JSON body — not a mock |
| `SearchInput placeholder` prop | `src/shared/components/SearchInput.tsx` | HTML placeholder attribute |
| Embedded payload catalog | `aisec-payload/src/library/mod.rs` | Real static payload library from `data/payloads.json` |
| Built-in attack payloads | `aisec-attack/src/attacks/prompt_injection.rs` | Real attack content, not test fakes |
| Rule/regex evaluators | `aisec-judge/src/evaluators/*` | Real deterministic evaluation |
| `LogOptions::bootstrap()` | `aisec-core/src/logging.rs` | Logging init helper name, not app mock |
...
|----------|-------------------|-------------|
| 1. Create project | `mockProjects`, New Project placeholder, `AppState` | `aisec-storage` + `project_create` IPC |
| 2. Add target | `mockTargets`, Add Target placeholder | `aisec-storage` + `target_create` IPC |
| 3. Run discovery | `mockDiscoveryJobs`, Start Scan placeholder | `aisec-discovery` via `scan_run` IPC |
| 4. Prompt injection | `mockAttackRuns`, Launch Attack placeholder | `aisec-attack` in `scan_run` pipeline |
| 5. Evaluate | `mockFindings`, status reducer | `aisec-judge` + `aisec-storage` findings repo |
| 6. HTML report | `mockReports`, Generate Report placeholder | `aisec-report` + `report_generate` IPC |
...
TEST DOUBLES
  crates/aisec-attack/src/transport/mock.rs
  crates/aisec-auth/src/mock.rs
  crates/aisec-models/src/runtime/mock.rs
  crates/aisec-judge/src/mock_runtime.rs
  crates/aisec-storage/src/pool.rs (test_utils)
  tests/frontend/logger.test.ts

MOCK SERVERS / FIXTURES
  scripts/discovery-test-target.py
  crates/aisec-discovery/tests/integration.rs
  crates/aisec-models/tests/integration.rs
