# PromptLab — Real Implementation Audit
...
> diverge. Notably, on this Linux toolchain `promptlab-models` and `promptlab-report` tests **pass**
> (the earlier report's macOS-specific `sysctl` failures do not occur), and `promptlab-plugin-host`
...
|---|--------|----------------|------------------------|
| 1 | `promptlab-core` | **COMPLETE** | Small, working error + logging foundation; both unit tests pass; only an ignored `json_file` flag. |
| 2 | `promptlab-storage` | **BROKEN** | Production SQLite library is real and complete-grade, but `cargo test` fails to compile (missing trait imports in 2 test modules). |
| 3 | `promptlab-discovery` | **BROKEN** | Detectors/HTTP/single-worker crawl are real, but the default multi-worker crawler deadlocks and a test hangs indefinitely. |
| 4 | `promptlab-auth` | **BROKEN** | Real Playwright subprocess + session design, but the crate does not compile (`tokio` `process`/`fs`/`io-util` features not enabled). |
| 5 | `promptlab-fingerprint` | **COMPLETE** | 59 real provider rules across 8 providers, documented scoring/penalties, all 15 tests pass. |
| 6 | `promptlab-payload` | **COMPLETE** | Real 24-entry payload library + real encoders + working pipeline; all 15 tests pass (optional `storage` feature unimplemented). |
| 7 | `promptlab-attack` | **BROKEN** | Coherent 9-attack framework with real HTTP transport, but `cargo test` fails to compile (`E0308` in a unit test); several budget/concurrency fields are dead. |
| 8 | `promptlab-models` | **PARTIAL** | Real HTTP download, SHA256 verify, and `llama-server` subprocess runtime; tests pass, but inference/download are never exercised live and vault import is incomplete. |
| 9 | `promptlab-judge` | **BROKEN** | Real rule/regex/LLM consensus engine, but a shipped integration test fails on a genuine consensus bug (spaced `API key:` slips below threshold). |
| 10 | `promptlab-report` | **PARTIAL** | HTML/JSON/SARIF/PDF formatters all real and tested (PDF via `printpdf`); PDF is cruder than documented (single page, no charts). |
| 11 | `promptlab-plugin-host` | **PARTIAL** | Real manifest/discovery/lifecycle/subprocess invocation, but 1 unit test fails, sample manifests break discovery, and sandbox/permissions are audit-only. |
...
**Overall:** PromptLab is a **strong library prototype with no integration spine**. The domain crates
...
|-------|--------------------------|--------|
| `promptlab-core` | ✅ 2 passed | — |
| `promptlab-storage` | ❌ compile error | `E0599: no method named create` (×2) |
| `promptlab-discovery` | ❌ hang | `crawler_respects_max_depth` never returns (worker deadlock) |
| `promptlab-payload` | ✅ 10 + 5 passed | — |
| `promptlab-models` | ✅ 12 + 5 passed | platform-specific HW detection uses `/proc/meminfo` on Linux |
| `promptlab-judge` | ❌ 1 failed | integration: `regex_and_rules_agree_on_secret` (2 passed, 1 failed) |
| `promptlab-report` | ✅ 9 + 5 passed | — |
| `promptlab-fingerprint` | ✅ 15 passed | — |
| `promptlab-plugin-host` | ❌ 1 failed | `permissions::tests::path_glob` (5 passed, 1 failed) |
| `promptlab-auth` | ❌ compile error | `E0432/E0433: tokio::process` not found |
...
### 1. `promptlab-core` — COMPLETE

Shared foundation: `error.rs` (`PromptLabError`, `ErrorCode`, `PromptLabResult`) and `logging.rs`
...
- **TODOs:** None.
- **Missing implementation:** **No encryption at rest** — `cookies_json`, `tokens_json`, `config_json`, file paths stored as plain TEXT (no `promptlab-vault`). `docs/DATABASE.md` still says "Version: 001" and omits the auth repository API (migration 002 exists and runs).
- **Blocking issues:** **`cargo test -p promptlab-storage` fails to compile (`E0599`).** Root cause: `finding.rs` and `attack_result.rs` test modules call `repos.scans().create(...)` but import only `ProjectRepository`, not the `ScanRepository` trait that defines `create`. The production library itself (`cargo build`) compiles fine — the failure is in the test harness.

---

### 3. `promptlab-discovery` — BROKEN
...
- **Blocking issues:** **`cargo test -p promptlab-auth` fails to compile (`E0432`/`E0433`).** The crate uses `tokio::process` (`playwright/client.rs:9-10`) plus `tokio::fs`/`AsyncBufReadExt`/`AsyncWriteExt`, but inherits the workspace `tokio` which enables only `["macros","rt-multi-thread","sync","time"]` — `process`/`fs`/`io-util` are not enabled.

---

### 5. `promptlab-fingerprint` — COMPLETE
...
- **TODOs:** None.
- **Missing implementation:** No coupling to `promptlab-discovery` snapshots (usage guidance only); `promptlab-core` is a declared-but-unused dependency; `fingerprint_batch()` is undocumented and untested.
- **Blocking issues:** None. All 15 tests pass (one detection test per provider + threshold/scoring tests).

---

### 6. `promptlab-payload` — COMPLETE
...
- **Blocking issues:** **`cargo test -p promptlab-attack` fails to compile (`E0308`).** `payload/runner.rs` test calls `PayloadRunner::new(transport)` with an owned value, but the constructor requires `&'a T` (production `executor.rs:77` correctly passes `&self.transport`). Compilation fails before any test runs.

---

### 8. `promptlab-models` — PARTIAL
...
- **Placeholder code:** Unused enum variants (`DownloadStatus::Paused`, `GpuBackend::Vulkan`/`Rocm`); non-Linux/macOS RAM fallback hardcodes 8 GiB; `recommended_gpu_layers()` returns flat `35`/`0` (no VRAM tuning); `lib.rs` doc says `~/.promptlab/models` but code uses `PROMPTLAB_MODEL_VAULT` or `./data/models`.
- **TODOs:** None.
...
- **TODOs:** None.
- **Missing implementation (vs `docs/JUDGE.md`):** Not referenced anywhere else in the workspace (not wired into `promptlab-attack`). Only 3 category rule sets exist (everything else falls back to `prompt_injection`); no RAG/tool/MCP-specific rules. LLM evaluator errors are silently skipped (debug log only).
...
### 11. `promptlab-plugin-host` — PARTIAL

Plugin host: `promptlab-plugin.toml` manifest parsing/validation, recursive discovery, lifecycle state
...
- **Production code (backend):** `src-tauri/src/{main,lib,state,error,logging}.rs` + `commands/mod.rs` — real Tauri bootstrap with `promptlab-core` logging and a `CommandError` IPC envelope.
...
- **Backend reality:** `src-tauri/Cargo.toml` depends only on `promptlab-core` — **no `promptlab-storage`, no engines**. `AppState` holds only a log guard; **no database is opened**, no engine is invoked.
...
The single most important finding: **the Tauri app links only `promptlab-core`.** None of the 10 domain
...
- **No encryption at rest:** `promptlab-storage` and `promptlab-auth` persist credentials/cookies/tokens as plain JSON; no `promptlab-vault`.
- **JWT is structure-only:** no signature/expiry verification (`promptlab-auth`).
- **Plugin sandbox is a bare subprocess:** no seccomp/cgroups/WASM; permission guard is audit-only; env stripping covers 2 keys.
- **SSRF guard is shallow:** hostname/literal-IP only; no DNS-rebinding or redirect re-validation (`promptlab-discovery`).

### Test suite cannot go green as-is
`cargo test --workspace` cannot pass without code changes: **3 compile failures** (`promptlab-storage`,
`promptlab-auth`, `tests/integration`), **1 hang** (`promptlab-discovery`), and **2 failing tests**
(`promptlab-judge`, `promptlab-plugin-host`). One additional compile failure (`promptlab-attack`) blocks its own
...
|----------|---------|----------|------|--------|
| `promptlab-core` | `promptlab-models` | *(Tauri backend, within `desktop-ui`)* | *(UI data layer, within `desktop-ui`)* | `promptlab-storage` |
| `promptlab-fingerprint` | `promptlab-report` | | | `promptlab-discovery` |
| `promptlab-payload` | `promptlab-plugin-host` | | | `promptlab-auth` |
| | `desktop-ui` | | | `promptlab-attack` |
| | | | | `promptlab-judge` |
